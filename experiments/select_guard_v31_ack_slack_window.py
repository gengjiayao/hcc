#!/usr/bin/env python3
"""Apply the frozen V31 ACK-slack window gates after mechanism admission."""
...
def select(campaign: Path) -> Dict[str, object]:
    ...
    if (spec.get("mechanism_profile") != "V31" or
            list(spec.get("seeds", [])) != [225, 226, 227, 228, 229] or
            set(dict(spec.get("arms", {}))) !=
                {"guard_k1", "guard_ack_slack_window", "hpcc", "homa"}):
        raise SelectionError("campaign is not the frozen V31 four-arm matrix")
    ...
    if (not sealed_spec_path.is_file() or
        ...
        raise SelectionError("campaign differs from the sealed V31 preflight")
    if (mechanism.get("phase") != "formal" or mechanism.get("passed") is not True or
        ...
        raise SelectionError("V31 performance is still sealed by the mechanism gate")
    workloads = dict(report.get("performance", {}))
    if set(workloads) != {"AliStorage40"}:
        raise SelectionError("V31 report must contain only AliStorage40")
    ...
    return {
        ...
        "status": "select_v31_for_independent_holdout" if selected else "retain_k1",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"V31 selection: {result['status']}")
    return 0
