#!/usr/bin/env python3
"""Apply the frozen V26 selected-elephant receiver-authority paired gates."""
...
try:
    ...
    from select_guard_v23_elephant_spillover import (
        SelectionError, read_object,
    )
...
def select(campaign: Path) -> Dict[str, object]:
    ...
    if (spec.get("mechanism_profile") != "V26" or
            list(spec.get("seeds", [])) != [200, 201, 202, 203, 204] or
            set(dict(spec.get("arms", {}))) !=
                {"guard_k1", "guard_elephant_authority"}):
        raise SelectionError("campaign is not the frozen V26 receiver-authority matrix")
    ...
    if (not sealed_spec_path.is_file() or
        ...
        raise SelectionError("campaign differs from the sealed V26 preflight")
    if (mechanism.get("phase") != "formal" or mechanism.get("passed") is not True or
        ...
        raise SelectionError("V26 performance is still sealed by the mechanism gate")
    performance = dict(report.get("performance", {}))
    if set(performance) != {"AliStorage50"}:
        raise SelectionError("V26 report must contain only AliStorage50")
    comparison = dict(dict(dict(performance["AliStorage50"])["paired"]).get(
        "guard_elephant_authority_minus_guard_k1", {}))
    ...
    for metric, field in PERCENT_GATES.items():
        ...
        gates[f"{metric}_percent_ci95_high_at_most_{limit}"] = (
            float(stats["ci95_high"]) <= limit)
    ...
    return {
        ...
        "candidate_arm": "guard_elephant_authority",
        "status": "select_elephant_authority_for_fresh_holdout" if selected
                  else "retain_k1",
        "selected_arm": "guard_elephant_authority" if selected else "guard_k1",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"V26 selection: {result['status']}")
    return 0
