...
from experiments.select_guard_v23_elephant_spillover import SelectionError, select


class GuardV23SelectionTests(unittest.TestCase):
    ...
    def campaign(self, root, long_high=-6.0):
        spec = {
            "mechanism_profile": "V23", "name": "v23",
            "seeds": [185, 186, 187, 188, 189],
            "arms": {"guard_k1": {}, "guard_spillover": {}},
            "selection": {
        ...
        self.write_json(root / "summary/general_report.json", {
            "performance": {"AliStorage50": {"paired": {
                "guard_spillover_minus_guard_k1": metrics}}}})

    def test_selects_only_after_crossing_homa_gap_gate(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            self.campaign(root)
            result = select(root)
            self.assertTrue(result["all_frozen_gates_passed"])
            self.assertEqual(result["selected_arm"], "guard_spillover")

    def test_rejects_smaller_development_improvement(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
