...
from experiments.select_guard_v27_initial_window_priority import PERCENT_GATES, select


class GuardV27SelectionTests(unittest.TestCase):
    ...
    def campaign(self, root, long_homa_high=-0.5):
        ...
        spec = {
            "mechanism_profile": "V27", "name": "v27",
            "seeds": [205, 206, 207, 208, 209],
            "arms": {
                "guard_k1": {}, "guard_initial_window": {},
                "hpcc": {}, "homa": {},
            },
            "selection": selection,
        }
        ...
        for comparison, metric, _field in PERCENT_GATES:
            high = -0.5
            if comparison == "guard_initial_window_minus_homa" and metric == \
                    "gt_1MB_fct_us_mean":
                high = long_homa_high
        ...
        comparisons["guard_initial_window_minus_guard_k1"][
    ...
    def test_selects_only_when_every_frozen_gate_passes(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(result["selected_arm"], "guard_initial_window")
