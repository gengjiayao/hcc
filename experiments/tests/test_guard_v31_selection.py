...
from experiments.select_guard_v31_ack_slack_window import PERCENT_GATES, select


class GuardV31SelectionTests(unittest.TestCase):
    def write_json(self, path, value):
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(json.dumps(value, sort_keys=True))

    def campaign(self, root, long_high=-0.5):
        ...
        spec = {"mechanism_profile": "V31", "name": "v31",
                "seeds": [225, 226, 227, 228, 229],
                "arms": {"guard_k1": {}, "guard_ack_slack_window": {},
        ...
        for comparison, metric, _field in PERCENT_GATES:
            high = long_high if (
                comparison == "guard_ack_slack_window_minus_homa" and
                metric == "gt_1MB_fct_us_mean") else -1.5
            paired.setdefault(comparison, {})[metric] = {"percent_vs_right": {
                "n": 5, "mean": high - 0.5, "ci95_low": high - 1.0,
                "ci95_high": high}}
        paired["guard_ack_slack_window_minus_guard_k1"][
    ...
    def test_pass_and_homa_long_failure(self):
