...
from experiments.select_guard_v22_elephant_aging import SelectionError, select


class GuardV22SelectionTests(unittest.TestCase):
    ...
    def campaign(self, root, queue_high=4.0):
        spec = {
            "mechanism_profile": "V22", "name": "v22",
            "seeds": [180, 181, 182, 183, 184],
            "arms": {"guard_k1": {}, "guard_aging": {}},
            "selection": {
        ...
        self.write_json(root / "summary/general_report.json", {
            "performance": {"AliStorage50": {"paired": {
                "guard_aging_minus_guard_k1": metrics}}}})

    def test_selects_aging_only_when_every_gate_passes(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            self.campaign(root)
            result = select(root)
            self.assertTrue(result["all_frozen_gates_passed"])
            self.assertEqual(result["selected_arm"], "guard_aging")
