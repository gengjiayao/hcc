#!/usr/bin/env python3
"""Apply the frozen V32 ACK-slack window gates after mechanism admission."""
...
try:
    ...
    from experiments.select_guard_v30_bounded_first_window import (
        PERCENT_GATES as V30_PERCENT_GATES,
    )
    ...
    from select_guard_v30_bounded_first_window import PERCENT_GATES as V30_PERCENT_GATES
    from select_guard_v29_post_grant_window_floor import _paired


PERCENT_GATES = tuple(
    (comparison.replace("guard_bounded_first_window", "guard_ack_slack_window"),
     metric, field)
    for comparison, metric, field in V30_PERCENT_GATES
)
...
def select(campaign: Path) -> Dict[str, object]:
    ...
    if (spec.get("mechanism_profile") != "V32" or
            list(spec.get("seeds", [])) != [230, 231, 232, 233, 234] or
            set(dict(spec.get("arms", {}))) !=
                {"guard_k1", "guard_ack_slack_window", "hpcc", "homa"}):
        raise SelectionError("campaign is not the frozen V32 four-arm matrix")
    ...
    if (not sealed_spec_path.is_file() or
        ...
        raise SelectionError("campaign differs from the sealed V32 preflight")
    if (mechanism.get("phase") != "formal" or mechanism.get("passed") is not True or
        ...
        raise SelectionError("V32 performance is still sealed by the mechanism gate")
    workloads = dict(report.get("performance", {}))
    if set(workloads) != {"AliStorage40"}:
        raise SelectionError("V32 report must contain only AliStorage40")
    ...
    comparison = "guard_ack_slack_window_minus_guard_k1"
    ...
    return {
        ...
        "candidate_arm": "guard_ack_slack_window", "control_arm": "guard_k1",
        "status": "select_v32_for_independent_holdout" if selected else "retain_k1",
        "selected_arm": "guard_ack_slack_window" if selected else "guard_k1",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"V32 selection: {result['status']}")
    return 0
