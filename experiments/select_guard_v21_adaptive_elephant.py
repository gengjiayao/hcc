#!/usr/bin/env python3
"""Apply the frozen V21 adaptive-elephant versus K=1 paired gates."""
...
try:
    ...
    from select_guard_v20_elephant_k import JAIN_GATE, JAIN_METRIC, PERCENT_GATES


class SelectionError(RuntimeError):
    """A V21 artifact or frozen selection contract is incomplete."""
...
def select(campaign: Path) -> Dict[str, object]:
    ...
    if (spec.get("mechanism_profile") != "V21" or
            list(spec.get("seeds", [])) != [175, 176, 177, 178, 179] or
            set(dict(spec.get("arms", {}))) != {"guard_k1", "guard_adaptive"}):
        raise SelectionError("campaign is not the frozen V21 adaptive matrix")
    ...
    if (mechanism.get("phase") != "formal" or mechanism.get("passed") is not True or
        ...
        raise SelectionError("V21 performance is still sealed by the mechanism gate")
    performance = dict(report.get("performance", {}))
    if set(performance) != {"AliStorage50"}:
        raise SelectionError("V21 report must contain only AliStorage50")
    paired = dict(dict(performance["AliStorage50"])["paired"])
    comparison = dict(paired.get("guard_adaptive_minus_guard_k1", {}))
    ...
    return {
        ...
        "candidate_arm": "guard_adaptive",
        "status": "select_adaptive_for_fresh_holdout" if selected else "retain_k1",
        "selected_arm": "guard_adaptive" if selected else "guard_k1",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"V21 selection: {result['status']}")
    return 0
