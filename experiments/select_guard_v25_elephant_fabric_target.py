#!/usr/bin/env python3
"""Apply the frozen V25 class-scoped fabric-target versus K=1 paired gates."""
...
def select(campaign: Path) -> Dict[str, object]:
    ...
    if (spec.get("mechanism_profile") != "V25" or
            list(spec.get("seeds", [])) != [195, 196, 197, 198, 199] or
            set(dict(spec.get("arms", {}))) !=
                {"guard_k1", "guard_elephant_target"}):
        raise SelectionError("campaign is not the frozen V25 elephant-target matrix")
    ...
    if (not sealed_spec_path.is_file() or
        ...
        raise SelectionError("campaign differs from the sealed V25 preflight")
    if (mechanism.get("phase") != "formal" or mechanism.get("passed") is not True or
        ...
        raise SelectionError("V25 performance is still sealed by the mechanism gate")
    performance = dict(report.get("performance", {}))
    if set(performance) != {"AliStorage50"}:
        raise SelectionError("V25 report must contain only AliStorage50")
    comparison = dict(dict(dict(performance["AliStorage50"])["paired"]).get(
        "guard_elephant_target_minus_guard_k1", {}))
    ...
    return {
        ...
        "candidate_arm": "guard_elephant_target",
        "status": "select_elephant_target_for_fresh_holdout" if selected
                  else "retain_k1",
        "selected_arm": "guard_elephant_target" if selected else "guard_k1",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"V25 selection: {result['status']}")
    return 0
