#!/usr/bin/env python3
"""Apply the frozen V24 fast-enter spillover versus K=1 paired gates."""
...
def select(campaign: Path) -> Dict[str, object]:
    ...
    if (spec.get("mechanism_profile") != "V24" or
            list(spec.get("seeds", [])) != [190, 191, 192, 193, 194] or
            set(dict(spec.get("arms", {}))) != {"guard_k1", "guard_spillover"}):
        raise SelectionError("campaign is not the frozen V24 spillover matrix")
    ...
    if (not sealed_spec_path.is_file() or
        ...
        raise SelectionError("campaign differs from the sealed V24 preflight")
    if (mechanism.get("phase") != "formal" or mechanism.get("passed") is not True or
        ...
        raise SelectionError("V24 performance is still sealed by the mechanism gate")
    performance = dict(report.get("performance", {}))
    if set(performance) != {"AliStorage50"}:
        raise SelectionError("V24 report must contain only AliStorage50")
    comparison = dict(dict(dict(performance["AliStorage50"])["paired"]).get(
        "guard_spillover_minus_guard_k1", {}))
    ...
    return {
        ...
        "status": "select_fast_spillover_for_fresh_holdout" if selected else "retain_k1",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"V24 selection: {result['status']}")
    return 0
