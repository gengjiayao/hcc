#!/usr/bin/env python3
"""Apply the frozen V27 initial-window-priority gates after mechanism admission."""
...
def select(campaign: Path) -> Dict[str, object]:
    ...
    if (spec.get("mechanism_profile") != "V27" or
            list(spec.get("seeds", [])) != [205, 206, 207, 208, 209] or
            set(dict(spec.get("arms", {}))) !=
                {"guard_k1", "guard_initial_window", "hpcc", "homa"}):
        raise SelectionError("campaign is not the frozen V27 four-arm matrix")
    ...
    if (not sealed_spec_path.is_file() or
        ...
        raise SelectionError("campaign differs from the sealed V27 preflight")
    if (mechanism.get("phase") != "formal" or mechanism.get("passed") is not True or
        ...
        raise SelectionError("V27 performance is still sealed by the mechanism gate")
    performance_by_workload = dict(report.get("performance", {}))
    if set(performance_by_workload) != {"AliStorage40"}:
        raise SelectionError("V27 report must contain only AliStorage40")
    ...
    jain_comparison = "guard_initial_window_minus_guard_k1"
    ...
    return {
        ...
        "candidate_arm": "guard_initial_window",
        "control_arm": "guard_k1",
        "status": "select_v27_for_independent_holdout" if selected else "retain_k1",
        "selected_arm": "guard_initial_window" if selected else "guard_k1",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"V27 selection: {result['status']}")
    return 0
