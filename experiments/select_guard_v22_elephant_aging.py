#!/usr/bin/env python3
"""Apply the frozen V22 fixed-K aging versus K=1 paired gates."""
...
class SelectionError(RuntimeError):
    """A V22 artifact or frozen selection contract is incomplete."""
...
def select(campaign: Path) -> Dict[str, object]:
    ...
    if (spec.get("mechanism_profile") != "V22" or
            list(spec.get("seeds", [])) != [180, 181, 182, 183, 184] or
            set(dict(spec.get("arms", {}))) != {"guard_k1", "guard_aging"}):
        raise SelectionError("campaign is not the frozen V22 aging matrix")
    ...
    if (mechanism.get("phase") != "formal" or mechanism.get("passed") is not True or
        ...
        raise SelectionError("V22 performance is still sealed by the mechanism gate")
    performance = dict(report.get("performance", {}))
    if set(performance) != {"AliStorage50"}:
        raise SelectionError("V22 report must contain only AliStorage50")
    paired = dict(dict(performance["AliStorage50"])["paired"])
    comparison = dict(paired.get("guard_aging_minus_guard_k1", {}))
    ...
    return {
        ...
        "candidate_arm": "guard_aging",
        "status": "select_aging_for_fresh_holdout" if selected else "retain_k1",
        "selected_arm": "guard_aging" if selected else "guard_k1",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"V22 selection: {result['status']}")
    return 0
